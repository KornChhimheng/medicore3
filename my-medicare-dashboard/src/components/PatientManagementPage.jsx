...
import { BriefcaseMedical, CheckCircle, UserPlus, UserMinus, Search, Plus, Eye, CalendarPlus } from 'lucide-react';
...
const PatientManagementPage = () => {
  ...
  const newPatientsToday = dummyPatients.filter(p => p.status === 'New' && p.lastVisit === new Date().toISOString().slice(0, 10)).length; // Assuming 'lastVisit' can indicate new registration if today's date
  const inactivePatients = dummyPatients.filter(p => p.status === 'Inactive').length;
  ...
  return (
    ...
    <div className="p-4 sm:p-6 lg:p-8 w-full bg-gray-50 min-h-screen font-inter">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-lg">
        <h2 className="text-3xl font-bold text-gray-800 mb-6">Patient Management Dashboard</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-6 rounded-xl shadow-md flex items-center space-x-4">
            ...
            <div className="flex-shrink-0 bg-blue-700 bg-opacity-30 rounded-full p-3 flex items-center justify-center">
              <BriefcaseMedical size={32} className="text-white" strokeWidth={1.8} /> {/* Lucide icon */}
            </div>
            <div>
          ...
          <div className="bg-gradient-to-br from-green-500 to-green-600 text-white p-6 rounded-xl shadow-md flex items-center space-x-4">
            ...
            <div className="flex-shrink-0 bg-green-700 bg-opacity-30 rounded-full p-3 flex items-center justify-center">
              <CheckCircle size={32} className="text-white" strokeWidth={1.8} /> {/* Lucide icon */}
            </div>
            <div>
          ...
          <div className="bg-gradient-to-br from-purple-500 to-purple-600 text-white p-6 rounded-xl shadow-md flex items-center space-x-4">
            ...
            <div className="flex-shrink-0 bg-purple-700 bg-opacity-30 rounded-full p-3 flex items-center justify-center">
              <UserPlus size={32} className="text-white" strokeWidth={1.8} /> {/* Lucide icon */}
            </div>
            <div>
          ...
          <div className="bg-gradient-to-br from-yellow-500 to-yellow-600 text-white p-6 rounded-xl shadow-md flex items-center space-x-4">
            ...
            <div className="flex-shrink-0 bg-yellow-700 bg-opacity-30 rounded-full p-3 flex items-center justify-center">
              <UserMinus size={32} className="text-white" strokeWidth={1.8} /> {/* Lucide icon */}
            </div>
            <div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0 sm:space-x-4">
          ...
          <div className="relative flex-grow w-full sm:w-auto">
            ...
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              {/* Search Icon */}
              <Search size={20} className="text-gray-400" strokeWidth={1.8} /> {/* Lucide icon */}
            </div>
          </div>
          ...
          <button className="flex items-center px-4 py-2 bg-green-500 text-white rounded-xl shadow-md hover:bg-green-600 transition duration-200 w-full sm:w-auto justify-center">
            {/* Plus Icon */}
            <Plus size={20} className="mr-2" strokeWidth={2.5} /> {/* Lucide icon */}
            Add New Patient
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredPatients.length > 0 ? (
            filteredPatients.map((patient) => (
              <div key={patient.id} className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 flex flex-col justify-between">
                <div className="flex items-center space-x-4 mb-4">
                  <img src={patient.avatar} alt={patient.name} className="w-12 h-12 rounded-full object-cover border-2 border-blue-200" />
                  <div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-4 pt-4 border-t border-gray-100 space-y-2 sm:space-y-0 sm:space-x-2">
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-semibold ${
                      ...
                      patient.status === 'New' ? 'bg-purple-100 text-purple-800' :
                  ...
                  <div className="flex space-x-2">
                    <button className="p-2 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200 transition duration-200" title="View Profile">
                      {/* View Profile Icon (Eye) */}
                      <Eye size={20} /> {/* Lucide icon */}
                    </button>
                    <button className="p-2 rounded-full bg-green-100 text-green-600 hover:bg-green-200 transition duration-200" title="Schedule Appointment">
