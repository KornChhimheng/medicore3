...
import com.hospital.management.api.features.hospitalmanagement.dto.request.DoctorRequest;
...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    @Override
    @Transactional
    public BaseResponse<Void> registerPatient(PatienRegister request) {
    ...
    @Override
    @Transactional
    public BaseResponse<Void> registerDoctor(DoctorRequest request) {
        BaseResponse<Void> response = new BaseResponse<>();
        try {
            User user = new User();
            Role roleWithPermissions = createRoleDoctorWithPermissions();
            user.setEmail(request.getEmail());
            user.setPassword(passwordEncoder.encode(request.getPassword()));
            user.setRoleId(roleWithPermissions.getId()); // Converts Role to Set<Role>
            user.setFirstName(request.getFirstName());
            ...
            doctor.setUserId(userResult.getId());
            ...
            response.setMessage("Create Doctor successfully");
            response.setStatus(GlobalConstant.SUCCESS);
            response.setCode(GlobalConstant.CODE);
            response.setData(null);
            return response;
    ...
    public BaseResponse<LoginResponse> login(LoginRequest request) {
        ...
        try {
            ...
            CustomUserDetail userInfo = (CustomUserDetail) authentication.getPrincipal();
    ...
    private Role createRoleDoctorWithPermissions() {
        // Ensure permissions exist or create them
        Permission read = permissionRepository.findByName("DOCTOR_READ")
                .orElseGet(() -> createPermission("DOCTOR_READ"));

        Permission write = permissionRepository.findByName("DOCTOR_WRITE")
                .orElseGet(() -> createPermission("DOCTOR_WRITE"));

        // Ensure role exists or create it with permissions
        Role userRole = roleRepository.findByName("DOCTOR")
                .orElseGet(() -> createRole("DOCTOR", Set.of(read, write)));
        return userRole;
    ...
    public RefreshTokenResponse refreshToken(RefreshTokenRequest request) {
        ...
        try {
            ...
            // jwtUtils.extractExpiration(request.getRefreshToken());
